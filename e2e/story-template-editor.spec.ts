...
test.describe('StoryTemplateEditor 追加写真（自由に動かせるステッカー）', () => {
  ...
  test('（回帰）エディタが開いた時点で、指が触れる前からbodyとhtml両方のtouch-actionが既に厳しくなっている', async ({ page }) => {
    // ドラッグを検知してからtouch-actionを切り替えるのでは遅い（ブラウザは指が
    // 触れた瞬間のtouch-action値でネイティブの引っ張り更新・戻る/進むスワイプ
    // ナビゲーションにするかを決めてしまうため）。エディタが開いている間は、
    // 実際にドラッグが始まる前から常にロックされている必要がある
    await page.goto('/?e2e=storyTemplateEditor');
    await page.waitForTimeout(1000);

    const locked = await page.evaluate(() => ({
      bodyTouchAction: document.body.style.touchAction,
      bodyOverscroll: (document.body.style as any).overscrollBehavior,
      htmlTouchAction: document.documentElement.style.touchAction,
      htmlOverscroll: (document.documentElement.style as any).overscrollBehavior,
    }));
    expect(locked.bodyTouchAction).toBe('none');
    expect(locked.bodyOverscroll).toBe('none');
    expect(locked.htmlTouchAction).toBe('none');
    expect(locked.htmlOverscroll).toBe('none');

    // 実際にドラッグしても途中で外れたりしない（そのまま維持される）ことも確認
    await page.evaluate(() => (window as any).__e2eAddPhotoLayer('photo_sticker_1'));
    await page.waitForTimeout(300);
    const layer = page.locator('[data-testid="layer-photo_sticker_1"]');
    const box = await layer.boundingBox();
    ...
    await page.waitForTimeout(30);
    const during = await page.evaluate(() => document.body.style.touchAction);
...
test.describe('StoryTemplateEditor テキスト編集中の背景スライド対策', () => {
  test('テキスト編集中だけbodyのoverflowが厳しくなり、終わると元に戻る（touch-actionはエディタが開いている間ずっと厳しいまま）', async ({ page }) => {
    await page.goto('/?e2e=storyTemplateEditor');
    await page.waitForTimeout(1000);
    ...
    // touch-actionはエディタが開いている間（visible）は常にロックされている
    expect(before.touchAction).toBe('none');
    ...
    expect(during.overflow).toBe('hidden');
  ...
  test('（回帰）body側の制限中でも、フォント一覧の実際の指スクロールは引き続き動く', async ({ page }) => {
    await page.goto('/?e2e=storyTemplateEditor');
    await page.waitForTimeout(1000);
    await page.getByTestId('story-editor-add-text-btn').click();
    await page.waitForTimeout(500);
    ...
    const scrollEl = page.getByTestId('font-dropdown-scroll');
    ...
    await page.waitForTimeout(50);
    for (let i = 1; i <= 8; i++) {
      await dispatchTouch(client, 'touchMove', [{ x: cx, y: cy - i * 15 }]);
