...
test.describe('StoryTemplateEditor テキスト編集中の背景スライド対策', () => {
  test('テキスト編集中だけbodyのtouch-action/overflowが厳しくなり、終わると元に戻る', async ({ page }) => {
    await page.goto('/?e2e=storyTemplateEditor');
    await page.waitForTimeout(1000);
    ...
    expect(before.touchAction).not.toBe('none');
    ...
    expect(after.touchAction).toBe(before.touchAction);
