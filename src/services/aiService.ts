...
export async function critiqueAccountFacts(factsText: string): Promise<AccountCritique> {
  ...
  const systemPrompt =
    ...
    '「良い傾向です」「工夫すると伸びます」のような具体性のない一般論は禁止です。' +
    '良い点・改善点のどちらも、必ず【実データ】の中の具体的な数値・項目名を引用しながら述べてください' +
    '（例：「投稿頻度が月2回にとどまっており、週1回のアカウントと比べて露出機会が半分以下」「金曜11時台の反応が他の時間帯の1.5倍あるのに、その時間の投稿がほぼない」）。' +
    '改善点は「何が弱く、なぜそれが問題で、次に何をすべきか」まで踏み込んで示してください。' +
    '他アカウントの実データは取得できないため、競合や市場価値については、' +
    '一般的にその業種・フォロワー規模で期待される水準（一般論・業界の目安）と、このアカウントの実データを具体的に数値で比較する形でコメントしてください。' +
...
このデータをもとに、以下のJSON形式で返してください（各項目は実データの具体的な数値・項目名を引用した、中身の濃い1〜2文にしてください）:
{
  "goodPoints": ["良い点1（実データのどの数値・項目が根拠かを明示）", "良い点2"],
  "improvementPoints": ["改善点1（具体的に何が弱く、なぜ問題で、どう直すべきか）", "改善点2"],
  "marketComment": "同業種・同規模のアカウントの一般的な水準と、このアカウントの実データを具体的に数値で比較したコメント（2〜3文）"
}`;
...
export async function chatWithAssistant(
  ...
  const staticInstructions =
    ...
    IG_CONTEXT +
    'ユーザーが写真・キャプション・投稿内容・アカウント運用について相談してきたら、当たり障りのない褒め方はせず、' +
    'プロの評論家として率直かつ厳しく分析してください。具体的には次の観点で評価します。' +
    '①写真の撮り方（構図・光・被写体の見せ方・統一感）②文章の書き方（冒頭のフック・読みやすさ・行動喚起・言葉選び）' +
    '③投稿の仕方（頻度・時間帯・ハッシュタグ・プロフィールとの一貫性）。' +
    ...
    '1つの相談は3〜10往復程度で結論が出るよう簡潔に進め、絵文字は控えめに。ただし「簡潔に」は「浅く」という意味ではないので、' +
    '指摘の具体性・厳しさは削らず、文章量よりも中身の濃さを優先してください。' +
    ...
    '実際に生成・投稿する操作は「投稿」タブ（フィード・ストーリー・リールの作成画面）で行うことをやんわり案内してください。';
  const todayInfo = (() => {
    const d = new Date();
