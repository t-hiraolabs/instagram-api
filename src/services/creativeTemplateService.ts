...
//
// フェーズ3の注記: templates(type='collage')も同じく0件だった。管理画面
// （AdminAssetsScreen.tsx）は「テンプレート管理はもう必要ありません」との判断で撤去済み
// のため、Collage型テンプレートも今後はStory型同様SQL直接投入でのみ追加されうる。
// 既存データが残る場合に備え、旧形式のまま読んだ場合でも変換できるようにしておく。
