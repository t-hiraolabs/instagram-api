...
# Expoアプリ本体は検索エンジンにインデックスさせたくない（ログイン前提でSPA＝クロール価値が無い）ので
# /app 配下に移動し、ルート（/）には検索流入を狙うLPを置く。
# 静的アセット（/_expo, /assets, /favicon.ico）はルート直下のまま絶対パス参照なので移動しない。
mkdir -p dist/app
