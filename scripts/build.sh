...
# 100vh/100%はモバイルのアドレスバー分の高さがずれるため、ブラウザ側で自動追従する100dvhを優先させる
sed -i 's%</head>%<link rel="manifest" href="/manifest.json"><link rel="apple-touch-icon" href="/icon.png"><meta name="apple-mobile-web-app-title" content="AImark"><meta name="mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="theme-color" content="#833AB4"><style>html,body{touch-action:pan-x pan-y;-ms-touch-action:pan-x pan-y;background:#0A0A0A}html,body,#root{height:100vh}@supports (height:100dvh){html,body,#root{height:100dvh}}</style><script>document.addEventListener("gesturestart",function(e){e.preventDefault()});document.addEventListener("gesturechange",function(e){e.preventDefault()});var _lt=0;document.addEventListener("touchend",function(e){var n=Date.now();if(n-_lt<=300){e.preventDefault()}_lt=n},{passive:false});document.addEventListener("wheel",function(e){if(e.ctrlKey)e.preventDefault()},{passive:false});</script></head>%' dist/index.html
